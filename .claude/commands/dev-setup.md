...
```bash
dotnet new webapi -n [name] --no-https false
```

**Blazor:**
```bash
dotnet new blazor -n [name]
```
...
**Rails:**
```bash
rails new [name] --database=postgresql --no-git
```
...
echo "node_modules/\n.env\n.env.local\n.DS_Store\ndist/\n.next/\n.nuxt/\n__pycache__/\nvenv/\n*.pyc\ntarget/\nbin/\nobj/" > .gitignore
git add .
git commit -m "chore: initial [Stack] scaffold"
```

**Variante B – Alles versionieren (Code + Projektdokumentation):**
```bash
# Im Projekt-Root (eine Ebene über dem Code-Verzeichnis):
git init
cat > .gitignore << 'EOF'
node_modules/
.env
...
EOF
git add .
git commit -m "chore: project setup – framework + [Stack] scaffold"
```
